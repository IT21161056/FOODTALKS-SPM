...
const createNewDelveryStatus = async (req, res) => {
  try {
    const { deliverPerson, orderId, cusname, state, date } = req.body;

    console.log("data >>>", deliverPerson, orderId, cusname, state, date);
    // await new Promise((resolve) => setTimeout(resolve, 3000));

    //confirm data
    if (!deliverPerson || !orderId || !cusname || !state) {
...
const updateDeliveryStatus = async (req, res) => {
  try {
    ...
    res.json({
      message: "Delivery Status details updated.",
      delivery: updatedDeliveryStatus,
    });
  } catch (error) {
    res.json({ message: error.message });
  }
